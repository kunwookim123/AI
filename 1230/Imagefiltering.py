...
# 테스트 이미지 생성
img = cv2.imread("image/Lenna_(test_image)")
...
# 컴볼루션을 해도 중앙 픽셀만 1배로 통과시키고 주변은 0이라 영향이 없다.
...
# 수직 엣지 x방향으로 밝기 변화 -> Sobel X로 검출
# 수평 엣지 y방향으로 밝기 변화 -> Sobel Y로 검출
